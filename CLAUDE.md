...
**The event vocabulary is a coupling** shared by `based-answers.py` (emitters), the `events` table, and `run.html`'s client JS: `phase` (searching|deterministic|semantic|coherence|passed|exhausted|error, round), `agent-line` (agent searcher|semantic|coherence, ANSI-stripped line), `claim-status` (index, claim, checking|pass|fail), `check-result` (deterministic|coherence, passed, output), `answer` (html fragment), `feedback` (round, text).
